<template>
    <el-container>
        ...
        <el-main class="view-operate-authorize-content">
            <el-skeleton :rows="7"
                         :loading="download"
                         animated>
...
import { QUERY_ROLES_URL, QUERY_LIMITES_URL, QUERY_OPERATORS_FOR_ROLE_URL, AUTHORIZE_OPERATORS_FOR_ROLE_URL } from '../config';
...
export default {
    ...
    created: async function() {
        const { username, password } = this.$store.getters,
            prefixes = [
                ...
                `${QUERY_LIMITES_URL}?username=${username}&password=${password}&navigator=false&index=0&count=`
            ];
    ...
    methods: {
        ...
        enterAuthorize: async function(role, limites) {
            ...
            try {
                res = await fetch([
                    AUTHORIZE_OPERATORS_FOR_ROLE_URL,
                    `?username=${username}`,
                    `&password=${password}`,
                    `&roleID=${role}`,
            ...
            this.$message({
                ...
                message: error ? result : '角色操作权限授权成功'
            });
        }
    }
};
